...
| 版本 commit | 分支上的提交图（不可变快照） | 仓库成员 | 建模者（历史 / diff / 回滚 / 合并） |
| **发布 release** | `commit × 环境` 的不可变指针 | 平台 | **消费方**（智能问数 / Agent / BI / 开放 API） |

**五条规则**

1. **版本以仓库为粒度**：一份文档 = 一个语义模型（同一份 OSSIE 文档里含 `ontology` 与
...
2. **版本是能力，不是独立功能**：提交 / 历史 / 分支 / 发布作为**上下文能力内嵌在「本体」与
   「语义模型」页面**（同一套组件、同一份仓库历史），不再占据顶层导航的独立席位。理由：
...
「历史」打开的是**仓库级**的提交图 + diff + 分支 + 合并；「发布」选择环境并填写说明。
两页看到的是同一份历史——因为版本本来就属于仓库，而不是属于某一个 section。

### 1.6 导航与初始状态（交互规则）

避免"点了没反应"的空转：进入应用后**不应要求用户先手动挑仓库和租户才能开始工作**。

1. 租户：默认使用 `default`；切换入口放在侧边栏底部（低频操作），不占顶栏主位。
2. 仓库：进入租户后**自动选中最近更新的仓库**（并记住上次选择）；只有存在多个仓库时
   才需要用户主动切换。
3. 空态引导：该租户下没有任何仓库时，点击「本体」/「语义模型」**直接进入创建流程**
   （跳到项目概览并自动打开"新建模型仓库"），而不是静默失效或只给一句提示。
4. 导航项在无仓库时**不置灰**——置灰会让用户不知道下一步该做什么；引导永远优于禁用。
