...
## 2026-09-24 — 数据源粒度：连接（租户级）+ 环境绑定（项目 × 环境）【⚠️ 试验性】

提问：「租户粒度高、项目对应具体业务，那数据源应该是什么粒度？一个部门可能有很多项目，
但数据库是同一份。」

**结论（文档）**

- `access-control.md` 新增 **§8 数据源与环境绑定的粒度**，并**标记为试验性**（⚠️）：
  标记原因是「凭证是否独立成实体」「命名空间挂绑定还是挂连接」两处仍可能变动。
  - 先拆词：现在的「数据源」塞了五件事（连接 / 凭证 / 命名空间 / 默认值 / 环境），
    它们的变化频率与归属都不同。
  - 判断法：把 §2.6 的「下沉红线」推广到项目维度——**数据库不因项目而变 → 连接不按项目复制；
    账号与 schema 常因项目而变 → 按项目分化的是绑定**。
  - 粒度：`连接`（租户级，登记一次）+ `绑定`（项目 × 环境 → 连接 + 命名空间 + 预留凭证）。
  - 为什么不是项目级数据源：同一库被 N 个项目用会变成登记 N 次、密码复制 N 份、轮换改 N 处；
    且**库是部门资产不是项目资产**。
  - 凭证是真正的难点：用「绑定可覆盖凭证」同时支持共享账号与每域账号两种实践。
  - 权限挂点：连接/凭证归租户管理员，**绑定归项目 Owner**（补上 §4.5 只讲了"凭证只进不出"、
    没讲"谁能改绑定"的缺口）。
- **修正文档不一致**：`design-ouline.md` §2.2 的绑定表键控原是 `(env, 逻辑名)`（**漏了项目维度**），
  已改为 `(项目, env, 逻辑名)`；并注明 §3.4 的「映射表 `(env, model, dataset)`」与 §2.2 的绑定表
  实为同一张表，现已合并为 `project_bindings`。

**实现（S2）**

- 新增 `src/binding.rs` + `project_bindings` 表
  （`project_id, environment, connection_id, credential_id(预留), namespace`，唯一键 `(项目, 环境)`）。
- API：`GET /api/projects/{id}/bindings`、`PUT/DELETE .../bindings/{env}`、
  **`POST /api/projects/{id}/semantic/deploy-to-binding`**。
- 校验：环境白名单；连接必须存在且**与项目同租户**（否则项目能指向别的租户的库）；
  命名空间留空回落连接的默认 schema；同环境重复绑定是替换不是新增。
- 前端：语义模型页「DDL / 部署」标签改为**绑定驱动**——
  环境绑定表（dev/test/prod 三行，未绑定可直接绑定）+ 按环境部署；部署不再要求每次临时选库填 schema。
  面板上带「试验性」徽章与说明。
- 单测 4 例：一份连接服务多项目、命名空间回落与替换、跨租户连接被拒、非法环境被拒。

**验证**：`cargo test`（26 单测 + 4 集成）、`cargo check --all-targets`、`npm run build` 全绿；
并用本地服务端到端验证了绑定场景（同连接两项目不同 schema、命名空间回落、非法环境、未绑定部署报错）。

**后续（未做）**：S3 凭证独立成表 + 轮换 + 引用计数；S4 逐表绑定覆盖（混合接入模式）。
