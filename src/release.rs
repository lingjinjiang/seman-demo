...
pub struct ReleaseRow {
    ...
    pub author: String,
    /// Per-project monotonic sequence. `created_at` only has millisecond
...
/// All releases of a project, newest first.
pub async fn list_releases(pool: &AnyPool, project_id: &str) -> ModelResult<Vec<ReleaseRow>> {
    Ok(sqlx::query_as::<_, ReleaseRow>(
        "SELECT * FROM releases WHERE project_id = ? ORDER BY seq DESC",
    )
    .bind(project_id)
    .fetch_all(pool)
    .await?)
...
pub async fn latest_release(
    ...
    Ok(sqlx::query_as::<_, ReleaseRow>(
        "SELECT * FROM releases WHERE project_id = ? AND environment = ?
...
pub async fn publish(
    ...
    let now = now_millis();
    sqlx::query(
        "INSERT INTO releases (id, project_id, environment, commit_id, message, author, seq, created_at)
         VALUES (?, ?, ?, ?, ?, ?, (SELECT COALESCE(MAX(seq), 0) + 1 FROM releases WHERE project_id = ?), ?)",
    )
    .bind(&id)
    .bind(project_id)
    .bind(environment)
...
mod tests {
    ...
    async fn publish_is_append_only_and_latest_wins() {
        ...
        let project = crate::vcs::create_project(&pool, "retail", None).await.unwrap();
        let head = project.head_branch_id.clone().unwrap();
        let branch = crate::vcs::get_branch(&pool, &project.id, &head).await.unwrap();
        let commit = branch.head_commit_id.clone().unwrap();

        assert!(latest_release(&pool, &project.id, "prod").await.unwrap().is_none());

        let first = publish(&pool, &project.id, "prod", &commit, Some("v1"), "alice")
            .await
            .unwrap();
        assert_eq!(first.environment, "prod");

        // Publishing again appends a record; history is preserved.
        publish(&pool, &project.id, "prod", &commit, Some("v2"), "bob")
            .await
            .unwrap();
        let all = list_releases(&pool, &project.id).await.unwrap();
        assert_eq!(all.len(), 2);
        assert_eq!(all[0].message.as_deref(), Some("v2"));
        assert_eq!(
            latest_release(&pool, &project.id, "prod")
                .await
                .unwrap()
                .unwrap()
                .message
        ...
        assert!(latest_release(&pool, &project.id, "dev").await.unwrap().is_none());
    }
    ...
    async fn unknown_environment_is_rejected() {
        let pool = test_pool().await;
        let project = crate::vcs::create_project(&pool, "retail", None).await.unwrap();
        let err = publish(&pool, &project.id, "staging", "c1", None, "alice").await;
        assert!(err.is_err());
    }
}
